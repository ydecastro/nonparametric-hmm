...
% ***********************************************************************
        % True transition matrix and stationnary law
        Qstar = [0.8 0.1 0.1;...
                 0.2 0.7 0.1;...
                 0.1 0.2 0.7];
        
         % Emission parameters (Beta laws)
         parambeta=[1.5 5; 6 6; 7 2];
...
%% Part I: Selection of m (computation of the minimal contrast values)
        ...
        % Parameters                   
        min_m=3;            % Lower bound on the size of the approximation 
                            % (size of the sieve)
        max_m=50;           % Upper bound on the size of the approximation 
                            % (in the paper max_m=n but we will see that 50 is largely enough)
...
fprintf(fileID,'M=%i %1.12f %i %1.12f\n',Columns); % Then run CAPUSHE !
...
%intercept
 p = polyfit((35:50)',-contrast_values(33:48),1);
 slope=p(1)*n;
%Slope = 26
...
%% Conclusion of the slope heuristic: m=25
    m=25;      
...
%-------------------------------------------------------------------------
            permut = [2 1 3];       % A modifier à la main.
%-------------------------------------------------------------------------
